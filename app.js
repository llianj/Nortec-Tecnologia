        ...
        async function saveClient() {
            ...
            } catch (err) { console.error(err); showToast('Erro ao salvar cliente: ' + (err.message || err.details || 'ver console'), 'error'); }
        }
        ...
        async function saveProduct() {
            ...
            } catch (err) { console.error(err); showToast('Erro ao salvar produto: ' + (err.message || err.details || 'ver console'), 'error'); }
        }
        ...
        async function saveService() {
            ...
            } catch (err) { console.error(err); showToast('Erro ao salvar serviço: ' + (err.message || err.details || 'ver console'), 'error'); }
        }
        ...
        async function saveEmployee() {
            ...
            } catch (err) { console.error(err); showToast('Erro ao salvar usuário: ' + (err.message || err.details || 'login já existe?'), 'error'); }
        }
        ...
        function removeOsPhoto(index) {
            currentOsPhotos.splice(index, 1);
            renderOsPhotosPreview();
        }
 
        function renderOsPhotosPreview() {
            const preview = document.getElementById('o-photos-preview');
            preview.innerHTML = '';
            currentOsPhotos.forEach((b64, idx) => {
                const wrapper = document.createElement('div');
                wrapper.className = 'relative w-24 h-24';
                wrapper.innerHTML = `
                    <img src="${b64}" class="w-24 h-24 object-cover rounded-lg border-2 border-green-500/50 shadow-lg">
                    <button type="button" onclick="removeOsPhoto(${idx})" title="Remover foto"
                        class="absolute -top-2 -right-2 w-6 h-6 bg-red-600 hover:bg-red-500 rounded-full flex items-center justify-center text-white shadow-lg border-2 border-[#1e293b]">
        ...
        async function saveOrder() {
            ...
            } catch (err) { console.error(err); showToast('Erro ao salvar OS: ' + (err.message || err.details || 'ver console'), 'error'); }
        }
        ...
        async function finalizePdvSale() {
            ...
            } catch (err) { console.error(err); showToast('Erro ao finalizar venda: ' + (err.message || err.details || 'ver console'), 'error'); }
        }
        ...
        async function saveExpense() {
            ...
            } catch (err) { console.error(err); showToast('Erro ao registrar despesa: ' + (err.message || err.details || 'ver console'), 'error'); }
        }
        ...
        async function saveSettings() {
            ...
            } catch (err) { console.error(err); showToast('Erro ao salvar configurações: ' + (err.message || err.details || 'ver console'), 'error'); }
        }
        ...
        function confirmDelete(type, id) {
            confirmAction = async () => {
                ...
                } catch (err) { console.error(err); closeConfirm(); showToast('Erro ao excluir: ' + (err.message || err.details || 'pode haver registros vinculados'), 'error'); }
            };
