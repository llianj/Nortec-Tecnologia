// =====================================================================
        ...
        function refreshAllViews() {
            ...
            renderOrders(); renderFinances(); renderDashboard();
        }
        ...
        function openTracking() {
            document.getElementById('login-screen').classList.add('hidden'); document.getElementById('tracking-screen').classList.remove('hidden'); document.getElementById('track-result').classList.add('hidden'); document.getElementById('track-id').value = '';
        }
        function closeTracking() {
            document.getElementById('tracking-screen').classList.add('hidden'); document.getElementById('login-screen').classList.remove('hidden');
        }

        // Rastreio público (sem login) — consulta direto o Supabase
        async function doTracking() {
            const id = parseInt(document.getElementById('track-id').value); if (!id) return;
            const res = document.getElementById('track-result'); res.classList.remove('hidden');
            ...
            try {
                ...
                if (error) throw error;
                if (!data) { res.innerHTML = '<p class="text-red-400 font-bold text-center"><i class="ph ph-warning"></i> OS não encontrada.</p>'; return; }
                const o = mapOrder(data);

                let statusCor = 'text-green-500'; if (o.status === 'Pendente' || o.status === 'Aguardando Peça') statusCor = 'text-amber-500'; else if (o.status === 'Em Andamento') statusCor = 'text-blue-500';

                res.innerHTML = `
                    <div class="border-b border-green-500/20 pb-3 mb-3">
                        <h3 class="font-bold text-white text-lg">OS #${padId(o.id)}</h3>
                        <p class="text-sm text-slate-300 font-medium">${o.equip}</p>
                    </div>
                    <div class="mb-3">
                        <p class="text-xs text-slate-400 uppercase tracking-wider font-bold">Status Atual</p>
                        <p class="text-xl font-black ${statusCor}">${o.status}</p>
                    </div>
                    <div class="mb-3">
                        <p class="text-xs text-slate-400 uppercase tracking-wider font-bold">Total do Serviço</p>
                        <p class="font-black text-white text-lg">${formatMoney(o.total)}</p>
                    </div>
                    ${o.problems ? `<div class="bg-[#1e293b] p-3 rounded-lg border border-green-500/10 mt-2"><p class="text-xs text-green-400 font-bold mb-1">Laudo Técnico:</p><p class="text-sm text-slate-300 italic">${o.problems}</p></div>` : ''}
                `;
            } catch (err) {
                console.error(err);
        ...
        function injectQRCode(idElement, osId) {
            setTimeout(() => {
                ...
                if (el) { el.innerHTML = ''; new QRCode(el, { text: "OS-" + osId, width: 80, height: 80, colorDark: "#000000", colorLight: "#ffffff" }); }
            }, 100);
        }
